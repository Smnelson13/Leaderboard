...
func processSaveScore(request: HTTPRequest, _ response: HTTPResponse)
{
    ...
    let theDisplayName = request.param(name: "displayname") else
  {
    ...
    responseDictionary["error"] = "Please seupply values"
    do {
  ...
  do {
    try aScore.save()
    responseDictionary["error"] = "No Error"
  }catch
  {
    print(error)
...
func getHighScores(request: HTTPRequest, response: HTTPResponse)
{
  response.setHeader(.contentType, value: "application/json")
  ...
  } catch //PerfectError.networkError
  {
  print(error)
  }
  response.completed()
